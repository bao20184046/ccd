...
Type* compileExpression2(void) {
  Type* type;
  type = compileTerm();
  Type* type_ = compileExpression3();
  if(type->typeClass==type_->typeClass)
    return type;
  else if((type->typeClass == TP_INT && type_->typeClass == TP_DOUBLE) || (type->typeClass == TP_DOUBLE && type_->typeClass == TP_INT))
  {
    type->typeClass=TP_DOUBLE;
    return type;
  }
  else error(ERR_TYPE_INCONSISTENCY, lookAhead->lineNo, lookAhead->colNo);
}
